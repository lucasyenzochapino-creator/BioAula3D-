          ...
          {showForm && (
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => exportarPDF({ id: "tmp", fecha, tema, texto })}
                  disabled={!texto.trim()}
                  className="px-4 py-2 rounded-xl bg-slate-700 hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed text-slate-200 text-sm font-medium transition-colors"
                >
                  PDF
                </button>
                <button
                  onClick={() => compartirComoPDF({ id: "tmp", fecha, tema, texto })}
                  disabled={!texto.trim()}
                  className="px-4 py-2 rounded-xl bg-slate-700 hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed text-slate-200 text-sm font-medium transition-colors"
                >
                  Compartir
